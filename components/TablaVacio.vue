<template>
  ...
  let overallGenero = ref(0);
  let overallMedioTrans = ref(0);
  ...
  const transformData = (dataDimensions: any[], contentTable: any[], cantidadRespuestas:number, 
    ...
    formatted.forEach(item => {
      if (item.level === 'dimension') {
        ...
        generosUnicos.forEach(genero => {
          ...
          if (generoData.count > 0) {
            item[`genero_${genero}`] = `${(((generoData.total * 100) / generoData.count)/generoMap[genero].length).toFixed(0)}%`;
            overallGenero.value += ((generoData.total * 100) / generoData.count)/generoMap[genero].length;
          }
        });
        medioTransporteUnico.forEach(data => {
    ...
    console.log(generoMap);
    const rowCount = generoMap[generosUnicos[0]].length;
    for (let i = 0; i < rowCount; i++) {
      ...
      generosUnicos.forEach((pregunta) => {
        const respuesta = generoMap[pregunta].length;  // Asegúrate de que `bdinfindxvc` existe
        row[`genero_${pregunta}`] = respuesta;
      });
  ...
  onMounted(async () => {
    ...
    // Inicializa `headers` y `formattedData` vacíos
    headers.value = [];
    formattedData.value = [];
